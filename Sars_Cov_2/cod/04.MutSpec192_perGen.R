...
for (gen in unique(ann$GenName)){
  ...
  fig <- ggplot(MutSpec12Comp,aes(x = NucSubst, y = MutSpec, ymin = 0, fill = NucSubst))+
    ...
    theme(legend.position="none")+
    theme(axis.text.x = element_text(size=3.2, angle = 90))+
    ggtitle(sprintf('Covid Mut spec 192 components in %s', gen))+
    #theme(axis.text.y = element_text(hjust = 3.4))+
    geom_text(aes(label=NucSubst), size = 1, angle = 90)
  fig %>% ggplotly
  ggsave(sprintf('D:/Sars_Cov_2_MutSpec-main/Sars_Cov_2_MutSpec-main/Sars_Cov_2/R_figure/04.MutSpec192_%s.svg', gen), plot = fig, device = 'svg',  limitsize = F)
}