...
def mutspec_192(data, ref):
    ...
    for index, row in second_nucl_data.iterrows():
        if ref[int(row['bp']) : int(row['bp'])+3] + ':' + ref[int(row['bp'])] + row['snp'] + ref[int(row['bp'])+3] in mutspec.keys():
            mutspec[ref[int(row['bp']) : int(row['bp'])+3] + ':' + ref[int(row['bp'])] + row['snp'] + ref[int(row['bp'])+3]] += row['snp_count']
        else:
            mutspec[ref[int(row['bp']) : int(row['bp'])+3] + ':' + ref[int(row['bp'])] + row['snp'] + ref[int(row['bp'])+3]] = row['snp_count']
    gen_mut_spec = pd.DataFrame([mutspec])
