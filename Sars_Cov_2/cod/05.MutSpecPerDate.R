...
for (x in unique(only_mut$Year)){
  ...
  for (i in unique(year_set$Month)){
    ...
    dev.off()
    write.csv(x = Month12Comp, sprintf('D:/Sars_Cov_2_MutSpec-main/Sars_Cov_2_MutSpec-main/Sars_Cov_2/data_obtained/MutPerMonth/05.Month_MutSpec12_%s.csv', paste(x,i,sep='-')))
  }
}
